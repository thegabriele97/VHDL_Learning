library IEEE;
use IEEE.STD_LOGIC_1164.ALL;
use ieee.numeric_std.all;

entity binomial is
    port(
        clk, rst, start: in std_logic;
        n, k: in std_logic_vector(2 downto 0);
        ready: out std_logic;
        data_out: out std_logic_vector(5 downto 0)
    );
end binomial;

architecture hlsm of binomial is

    type fsm_state is ( wait_start, check, calc, ultimate, done );
    signal curr_state, next_state: fsm_state;

    signal curr_r0, next_r0: std_logic_vector(14 downto 0);
    signal curr_r1, next_r1: std_logic_vector(12 downto 0);
    signal curr_n, next_n, curr_k, next_k: std_logic_vector(2 downto 0);

begin

    data_out <= curr_r0(5 downto 0);

    process(curr_state, curr_r0, curr_r1, curr_n, curr_k, n, k, start)
    begin
    
        next_state <= curr_state;
        next_r0 <= curr_r0;
        next_r1 <= curr_r1;
        next_n <= curr_n;
        next_k <= curr_k;
        ready <= '0';
        
        case curr_state is
        
            when wait_start =>
                next_r0 <= std_logic_vector(TO_UNSIGNED(1, next_r0'length));
                next_r1 <= std_logic_vector(TO_UNSIGNED(1, next_r1'length));
                next_n <= n;
                next_k <= k;
                
                if (start = '1') then
                    next_state <= check;
                end if;
                
            when check =>
                next_state <= calc;
                if (unsigned(curr_k) = 0) then
                    next_state <= ultimate;
                end if;
                
            when calc =>
                next_r0 <= std_logic_vector(unsigned(curr_r0(11 downto 0)) * unsigned(curr_n));
                next_r1 <= std_logic_vector(unsigned(curr_r1(9 downto 0)) * unsigned(curr_k));
                next_n <= std_logic_vector(unsigned(curr_n) - 1);
                next_k <= std_logic_vector(unsigned(curr_k) - 1);
                next_state <= check;
            
            when ultimate =>
                next_r0 <= std_logic_vector(unsigned(curr_r0) / unsigned(curr_r1));
                next_state <= done;
                
            when done =>
                ready <= '1';
                next_state <= wait_start;
            
            when others =>
                next_state <= wait_start;
        
        end case;
    
    end process;

    process(clk, rst)
    begin
    
        if (rst = '1') then
            curr_state <= wait_start;
            curr_r0 <= (others => '0');
            curr_r1 <= (others => '0');
            curr_n <= (others => '0');
            curr_k <= (others => '0');
        elsif (rising_edge(clk)) then
            curr_state <= next_state;
            curr_r0 <= next_r0;
            curr_r1 <= next_r1;
            curr_n <= next_n;
            curr_k <= next_k;
        end if;
    
    end process;

end hlsm;

architecture fsmd of binomial is

    -- Datapath signals
    signal curr_r0, next_r0: std_logic_vector(14 downto 0);
    signal curr_r1, next_r1: std_logic_vector(12 downto 0);
    signal curr_n, next_n, curr_k, next_k: std_logic_vector(2 downto 0);
    
    -- Controller signals
    type fsm_state is ( wait_start, check, calc, ultimate, done );
    signal curr_state, next_state: fsm_state;    
    
    -- Control signals
    signal mux_r0, mux_r1, mux_n, mux_k: std_logic_vector(1 downto 0);
    signal stop: std_logic;

begin

    data_out <= curr_r0(5 downto 0);

    -- Datapath descritpion
    process(curr_r0, curr_r1, curr_n, curr_k, mux_r0, mux_r1, mux_n, mux_k, n, k)
    begin
    
        -- R0 register
        next_r0 <= curr_r0;
        if (mux_r0 = "01") then
            next_r0 <= std_logic_vector(unsigned(curr_r0(11 downto 0)) * unsigned(curr_n));
        elsif (mux_r0 = "10") then
            next_r0 <= std_logic_vector(unsigned(curr_r0) / unsigned(curr_r1));
        elsif (mux_r0 = "11") then
            next_r0 <= std_logic_vector(TO_UNSIGNED(1, next_r0'length));
        end if;
        
        -- R1 register
        next_r1 <= curr_r1;
        if (mux_r1 = "01") then
            next_r1 <= std_logic_vector(unsigned(curr_r1(9 downto 0)) * unsigned(curr_k));
        elsif (mux_r1 = "10") then
            next_r1 <= std_logic_vector(TO_UNSIGNED(1, next_r1'length)); 
        end if;       
        
        -- N register
        next_n <= curr_n;
        if (mux_n = "01") then
            next_n <= std_logic_vector(unsigned(curr_n) - 1);
        elsif (mux_n = "10") then
            next_n <= n;
        end if;
        
        -- K register
        next_k <= curr_k;
        if (mux_k = "01") then
            next_k <= std_logic_vector(unsigned(curr_k) - 1);
        elsif (mux_k = "10") then
            next_k <= k;
        end if;         
    
        -- Stop logic
        stop <= '0';
        if (unsigned(curr_k) = 0) then
            stop <= '1';
        end if;
    
    end process;
    
    process(clk, rst)
    begin
    
        if (rst = '1') then
            curr_r0 <= (others => '0');
            curr_r1 <= (others => '0');
            curr_n <= (others => '0');
            curr_k <= (others => '0');
        elsif (rising_edge(clk)) then
            curr_r0 <= next_r0;
            curr_r1 <= next_r1;
            curr_n <= next_n;
            curr_k <= next_k;
        end if;
    
    end process;
    
    -- Controller description
    process(curr_state, stop, start)
    begin
    
        next_state <= curr_state;
        mux_r0 <= "00";
        mux_r1 <= "00";
        mux_n <= "00";
        mux_k <= "00";
        ready <= '0';
        
        case curr_state is
        
            when wait_start =>
                mux_r0 <= "11";
                mux_r1 <= "10";
                mux_k <= "10";
                mux_n <= "10";
                
                if (start = '1') then
                    next_state <= check;
                end if;
                
            when check =>
                next_state <= calc;
                if (stop = '1') then
                    next_state <= ultimate;
                end if;
            
            when calc =>
                mux_r0 <= "01";
                mux_r1 <= "01";
                mux_n <= "01";
                mux_k <= "01";
                next_state <= check;
                
            when ultimate =>
                mux_r0 <= "10";
                next_state <= done;
                
            when done =>
                ready <= '1';
                next_state <= wait_start;
                
            when others =>
                next_state <= wait_start;
        
        end case;
    
    end process;
    
    process(clk, rst)
    begin
    
        if (rst = '1') then
            curr_state <= wait_start;
        elsif (rising_edge(clk)) then
            curr_state <= next_state;
        end if;
    
    end process;

end fsmd;
